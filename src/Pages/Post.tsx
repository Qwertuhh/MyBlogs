...
import Image from "../appwrite/imagesBucket";
...
export default function Post() {
  ...
  useEffect(() => {
    if (slug) {
      Database.getPost(slug).then((post) => {
        if (post) setPost(post as unknown as Data);
        else navigate("/");
      });
    } else navigate("/");
  }, [slug, navigate]);
  ...
  const deletePost = () => {
    Database.deletePost(post!.$id).then((status) => {
      if (status) {
        Image.deleteImage(post!.featuredImage);
        navigate("/");
      }
    });
  };
    ...
    <div className="py-8">
      <Container>
        <div className="w-full flex justify-center mb-4 relative border rounded-xl p-2">
          <img
            src={Image.getImage(post.featuredImage)}
            alt={post.title}
